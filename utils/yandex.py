    ...
    async def get_excel(cls, cnx):
        ...
        for point in latest_checks.index:
            if grouped.get_group(point).shape[0] > 1:
                ...
                same_time_checks = grouped.get_group(point)[grouped.get_group(
                    point)['date_time'] == latest_checks.loc[point, 'date_time']]
                # Сортируем по количеству символов в 'comment' в обратном порядке
                same_time_checks = same_time_checks.sort_values(
                    by=['comment'], key=lambda x: x.str.len(), ascending=False)
                # Выбираем первый чек
                latest_checks.loc[point] = same_time_checks.iloc[0]
        latest_checks["FD/shift"] = (latest_checks["FD"] /
                                     latest_checks["shift_number"]).round()
        yandex_df[['Широта', 'Долгота']
                  ] = latest_checks['coordinates'].str.split(' ', expand=True)
        yandex_df["Широта"] = yandex_df["Широта"].astype(
            str).str.replace(",", "")
        yandex_df["random"] = np.random.uniform(
            low=-9/100000, high=9/100000, size=len(yandex_df))
        yandex_df["Широта"] = yandex_df["Широта"].astype(
            float) + yandex_df["random"]
        yandex_df["random"] = np.random.uniform(
            low=-9/100000, high=9/100000, size=len(yandex_df))
        yandex_df["Долгота"] = yandex_df["Долгота"].astype(
            float) + yandex_df["random"]
        yandex_df["Описание"] = "Адрес = " + latest_checks["address"].astype(str) + "    " + "дата и время = " + latest_checks["date_time"].astype(
            str) + "    "+"комментарий = " + latest_checks["comment"].astype(str)
