...
handler = logging.FileHandler(
    f"log/{__name__}.log", mode='w', encoding="UTF-8")
...
async def await_load(message: Message, state: FSMContext):
    ...
    if file_name:
        if file_name.split(".")[-1] == "xlsx":
            ...
            logger.info(
                f"{message.from_user.username} excel таблица прочитана")
...
async def add_cheque(message: Message, state: FSMContext):
    ...
    if response["coordinates"] is None:
        logger.warning(
            f"{message.from_user.username} оставляем адрес как есть, статус 2")
        await state.update_data(coordinates="---")
        await state.update_data(status=2)
        ic("status 2")
    else:
        logger.info(
            f"{message.from_user.username} нашли координаты, адрес правильный, статус 1")
        ic("status 1")
...
async def update_address(message: Message, state: FSMContext):
    ...
    if coordinates is None:
        logger.warning(
            f"{message.from_user.username} не удалось распознать адрес, статус 2")
        ...
        logger.info(
            f"{message.from_user.username} координаты распознаны, спрашиваем оставить ли")
...
async def check_agree(call: CallbackQuery, state: FSMContext):
    if call.data == "Yes":
        logger.info(
            f"{call.from_user.username} оставляем правильные координаты, статус 1")
        ...
        if data["type_update"] == "add address":
            ...
            with rq:
                rq.write_update(
                    "cards", [("address", data["right_address"])], f'id={data["id"]}')
                rq.write_update(
                    "cards", [("coordinates", data["right_coordinates"])], f'id={data["id"]}')
                rq.write_update("cards", [("status", 1)], f'id={data["id"]}')
                res = rq.select_one("cards", ["@*"], f'id={data["id"]}')
            columns = ("id", "id_telegram", "user_name_telegram", "company_name", "date_time",
                       "address", "cheque_number", "FD", "shift_number", "coordinates", "status", "comment")
    ...
    if call.data == "No":
        logger.info(
            f"{call.from_user.username} пробуем ещё раз или оставляем неправильный адрес")
...
async def try_again_address(call: CallbackQuery, state: FSMContext):
    if call.data == "Yes":
        ...
        if data["type_update"] == "add address":
            ...
            await call.message.delete()
            logger.info(
                f"{call.from_user.username} спрашиваем название компании")
            with rq as cursor:
                tmp_company_names = cursor.execute("""SELECT company_name, COUNT(*) AS count
                    FROM "cards"
                    GROUP BY company_name
            ...
            with rq:
                rq.write_update(
                    "cards", [("address", data["incorrect_address"])], f'id={data["id"]}')
                rq.write_update(
                    "cards", [("coordinates", "---")], f'id={data["id"]}')
...
async def callback_Yes(call: CallbackQuery, state: FSMContext):
    logger.info(
        f"{call.from_user.username} да, нужен, спрашиваем комментатрий")
...
async def callback_No(call: CallbackQuery, state: FSMContext):
    logger.info(
        f"{call.from_user.username} нет, не нужен, спрашиваем все ли корректно?")
...
async def callback_Yes_right(call: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    logger.info(
        f"{call.from_user.username} да, все верно, загружаем в базу данных")
    # Загрузить в бд
    with rq:
...
async def await_search(message: Message, state: FSMContext):
    ...
    with rq:
        res = rq.select_many(
            "cards", ["id", "company_name", "address"], f'address LIKE "%{message.text}%"')
    if not res:
        await message.answer("По этому запросу ничего не найдено, попробуйте ещё раз")
        return
    logger.info(f"{message.from_user.username} нашёл {
                len(res)} карточек по запросу {message.text}")
...
async def get_card(call: CallbackQuery, state: FSMContext):
    ...
    logger.info(f"{call.from_user.username} берёт карточку с id: {
                call.data.split()[-1]}")
    with rq:
...
async def button(call: CallbackQuery, state: FSMContext):
    ...
    if "address" in call.data:
        ...
        logger.info(
            f"{call.from_user.username} хочет отредактировать название компании")
        ...
        logger.info(
            f"{call.from_user.username} хочет отредактировать комментарий")
...
async def edit_await(message: Message, state: FSMContext):
    state_data = await state.get_data()
    logger.info(f"{message.from_user.username} отредактировал {
                state_data["edit"]} на {message.text} у карточки с id:{state_data["id"]}")
    with rq:
        if message.text != "Отмена ❌":
            rq.write_update("cards", [(state_data["edit"], message.text)], f'id = {
                            state_data["id"]}')
...
async def back(call: CallbackQuery, state: FSMContext):
    ...
    if data["type"] == "id":
        with rq:
            res = rq.select_many(
                "cards", ["id", "company_name", "address"], f'id_telegram = {call.from_user.id}')
    elif data["type"] == "text":
        with rq:
            res = rq.select_many(
                "cards", ["id", "company_name", "address"], f'address LIKE "%{data["filter"]}%"')
...
async def list_my_cards(message: Message, state: FSMContext):
    ...
    with rq:
        res = rq.select_many("cards", ["id", "company_name", "address"], f'id_telegram = {
...
async def send_excel_tb(message: Message, state: FSMContext):
    if str(message.from_user.id) in ADMIN:
        logger.info(
            f"{message.from_user.username} спрашиваем скачать или загрузить таблицу")
...
async def load(call: CallbackQuery, state: FSMContext):
    ...
    if call.data == "Download":
        logger.info(
            f"{message.from_user.username} хочет получить таблицу excel")
        with rq:
            try:
